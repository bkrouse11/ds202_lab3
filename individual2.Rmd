...
data %>%
  ...
  mutate(Happy = ifelse(`General happiness` == "Very happy" | 
                        `General happiness` == "Pretty happy", "Happy", "Unhappy")) %>%
  group_by(`Belief in life after death`) %>%
  summarize(`Percentage happy` = 100 * length(Happy[which(Happy == "Happy")]) / n(), .groups="drop_last") %>%
  ungroup() %>%
  group_by(`Belief in life after death`) %>%
  summarize(`Change in happiness` = diff(`Percentage happy`) / diff(`Gss year for this respondent`))
```

This investigation looks at how people's belief in life after death has affected their happiness over time. A person was considered to be `Happy` if they responded to the questionnaire that they considered themselves to be either `Very happy` or `Pretty happy`. 
